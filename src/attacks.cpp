...
{
    MagicalBitboards::MagicalBitboards()
    {
        initSliderAttacks();
    }
    ...
    uint64_t MagicalBitboards::maskBishopAttacksWithBlock(uint8_t position, uint64_t boardMask)
    {
        uint64_t attacks = 0ULL;
    ...
    uint64_t MagicalBitboards::maskRookAttacksWithBlock(uint8_t position, uint64_t boardMask)
    {
        uint64_t attacks = 0ULL;
        uint64_t attack = 0ULL;
        char r, f;
    ...
    uint64_t MagicalBitboards::setOccupancy(uint32_t position, uint8_t bitCount, uint64_t attackMask)
    {
    ...
    uint64_t MagicalBitboards::findMagicNumber(uint32_t position, uint8_t bitCount,
        ...
        std::function<uint64_t(uint8_t, uint64_t)> attacksWithBlockFunction)
    {
        ...
        for (int random_count = 0; random_count < 100000000; random_count++)
        {
            ...
            {
                int magicIndex = (int)((occupancies[index] * magicNumber) >> (64 - bitCount));
    ...
    void MagicalBitboards::initSliderAttacks()
    {
        for (int square = 0; square < 64; square++)
        {
            ...
            for (int i = 0; i < occupancyIndicies; i++)
            {
                ...
                int magicIndex = (occupancy * m_bishopMagicBitboard[square]) >> (64 - m_occupacyCountBishop[square]);
                m_bishopAttacks[square][magicIndex] = maskBishopAttacksWithBlock(square, occupancy);
            }
            ...
            {
                uint64_t occupancy = setOccupancy(i, relevantBitsCount, attackMask);
                int magicIndex = (occupancy * m_rookMagicBitboard[square]) >> (64 - m_occupacyCountRook[square]);
                m_rookAttacks[square][magicIndex] = maskRookAttacksWithBlock(square, occupancy);
            }
        }
    }
