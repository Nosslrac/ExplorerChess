...
void MoveGen::generateKingMoves(const Position &pos, MoveList &move_list) const
{
...
void MoveGen::generatePawnMoves(const Position &pos, MoveList &move_list) const
{
...
void MoveGen::generatePieceMoves(const Position &pos, MoveList &move_list) const
{
                             ...
                             uint8_t EP) const
{
  ...
  const bitboard_t boardWithoutEP = (realPawn | capturers) ^ pos.teamBoards[0];
...
inline void MoveGen::makeCaptureMove(const bitboard_t pieceBoards[],
                                     ...
                                     uint8_t from, uint32_t flagAndPiece) const
{
  while (toSQs)
  {
    int toSQ = bitScan(toSQs);
