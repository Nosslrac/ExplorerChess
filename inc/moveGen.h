...
class MoveGen : magicalBits::MagicalBitboards {
  ...
  private:
    ...
    void makePawnMove(const Position &pos, MoveList &move_list,
    ...
    inline void makePieceMove(MoveList &move_list, bitboard_t toSQs,
    ...
    constexpr inline bitboard_t pawnAttackBB(bitboard_t squareBB) const {
        return shift<white, UP_LEFT>(squareBB) |
               shift<white, UP_RIGHT>(squareBB);
    }
