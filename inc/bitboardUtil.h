...
template <Piece p> inline uint64_t pieces(const uint64_t pieces[]) {
...
template <bool whiteToMove, Direction D> uint64_t shift(uint64_t b) {
...
template <bool white> inline uint64_t forwardSquares(uint8_t sq) {
    if constexpr (white) {
...
// Castling is blocked
template <bool white, bool kingSide> bool isOccupied(uint64_t board) {
    if constexpr (white) {
...
template <bool white, bool kingSide> bool isAttacked(uint64_t attack) {
    if constexpr (white) {
...
template <bool whiteToMove> inline uint64_t EPpawns(const Position &pos) {
    if constexpr (whiteToMove)
        return Rank4 & pos.pieceBoards[0];
    return Rank5 & pos.pieceBoards[5];
}

inline bool moreThanOne(uint64_t b) { return b & (b - 1); }
...
constexpr uint64_t files[8] = {0x0101010101010101ULL, 0x0202020202020202ULL,
...
constexpr uint64_t main_diagonals[15] = {0x0100000000000000ULL,
                                         ...
                                         0x8040ULL,
                                         0x80ULL};
constexpr uint64_t anti_diagonals[15] = {0x1ULL,
                                         0x0102ULL,
                                         0x010204ULL,
                                         0x01020408ULL,
                                         0x0102040810ULL,
...
constexpr uint64_t adjacentFiles[8] = {files[0] | files[1],
